...
async function handleCreateLocalBooking(req, res) {
  try {
    ...
    sendJson(res, 201, {
      message: 'Booking stored in MongoDB.',
...
async function handleLocalBookings(req, res) {
  try {
    ...
    if (walletAddress) {
      ...
      filter.walletAddress = normalizeAddress(walletAddress);
    }
    ...
    sendJson(res, 500, { error: error.message || 'Unable to fetch MongoDB bookings.' });
  }
}
...
async function routeRequest(req, res) {
  ...
  if (pathname === '/health') {
    ...
    try {
      await connectMongo();
      localBookingCount = await Booking.countDocuments();
      database = 'connected';
    } catch {
      database = 'error';
    }
